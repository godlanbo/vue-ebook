  ...
  <div class="flap-card-wrapper" v-show="flapCardVisible">
    <div class="flap-card-bg" :class="{'animation' : runFlapCardAnimation}" v-show="runFlapCardAnimation">
      ...
      <div class="point-wrapper">
        <div
          class="point"
          :class="{'animation': runPointAniamtion}"
  ...
  methods: {
    ...
    next() {
      ...
      this.flapCardList.forEach((item, index) => {
        item.zIndex = 96 + ((this.front - index + 4) % len)
      })
      this.prepare()
    },
    prepare() {
      ...
      backFlapCard._g = backFlapCard.g - 5 * 9 * 2
      this.rotate(this.back, 'back')
    },
    ...
    startPointAnimation() {
      this.runPointAniamtion = true
      setTimeout(() => {
        this.runPointAniamtion = false
      }, 750)
    },
    stopAnimation() {
      if (this.task) {
