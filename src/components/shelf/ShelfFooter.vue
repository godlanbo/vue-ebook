...
export default {
  ...
  methods: {
    ...
    setPrivate() {
      ...
      this.shelfSelected.forEach((book) => (book.private = isPrivate))
      this.onComplete()
      if (isPrivate) {
        this.simpleToast(this.$t('shelf.setPrivateSuccess'))
      } else {
        this.simpleToast(this.$t('shelf.cancelPrivateSuccess'))
      }
    },
    async setDownload() {
      this.onComplete()
...
.shelf-footer {
  ...
  box-shadow: px2rem(0) px2rem(-2) px2rem(4) px2rem(0) rgba(0, 0, 0, 0.1);
